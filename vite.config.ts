...
function devApiPlugin(): Plugin {
  return {
    ...
    configureServer(server) {
      ...
      server.middlewares.use('/api/promotions/claim', async (req, res) => {
        try {
          ...
          const host = req.headers.host || 'localhost:3000';
          const webRequest = new Request(`http://${host}/api/promotions/claim`, {
            method,
            headers,
            body,
          });
          const api = await import('./api/promotions/claim.js');
          const response = method === 'OPTIONS'
            ? await api.OPTIONS(webRequest)
            : method === 'POST'
              ? await api.POST(webRequest)
          ...
          const message = error instanceof Error ? error.message : 'Unable to claim promotion coupon';
          res.writeHead(500, { 'Content-Type': 'application/json' });
          res.end(JSON.stringify({ error: message }));
        }
      });
