 ...
 * 提供購物車安全保存安心提示、重試結帳按鈕、返回首頁/購物車按鈕，以及客服定位用的錯誤追蹤碼。
 */
export const CheckoutErrorFallback: React.FC<CheckoutErrorFallbackProps> = ({
  ...
  return (
    <div className="w-full max-w-xl mx-auto my-8 p-6 sm:p-8 bg-white rounded-3xl border border-rose-100 shadow-xl shadow-rose-500/5 text-center">
      {/* 警告與購物車守護盾牌圖示 */}
      <div className="relative w-16 h-16 mx-auto mb-5">
        ...
        <div className="absolute -bottom-1 -right-1 bg-brand text-white p-1 rounded-full shadow">
      ...
      <div className="bg-blush/80 border border-blush rounded-xl p-3.5 mb-6 flex items-center justify-center gap-2 text-brand text-xs sm:text-sm">
      ...
      <div className="mb-6 p-3 bg-stone-50 border border-stone-200 rounded-xl flex items-center justify-between gap-2 max-w-md mx-auto">
        <div className="flex items-center gap-2 text-xs font-mono text-stone-600 truncate">
          ...
          <span className="font-semibold text-stone-900 select-all">{displayEventId}</span>
        </div>
        <button
          type="button"
          onClick={handleCopyEventId}
          className="text-xs px-2.5 py-1 bg-white border border-stone-200 rounded-lg text-stone-700 hover:bg-stone-100 transition-colors flex-shrink-0 font-medium"
        >
          {copied ? '已複製 ✓' : '複製代碼'}
        </button>
      </div>
