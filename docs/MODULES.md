...
| 訂單 | 導向真實訂單來源，不顯示示範個資 | 已驗證會員 session 建立可信 Cart link；Shopify HMAC、Webhook ID 去重、事件時間戳防倒退；`orders`／`order_items` 僅允許會員讀自己的列。`verify:commerce` 要求 success／failed／cancelled 三種案例的 Order ID、金額、付款、物流與發票跨系統一致 | App、secret 與五個既有訂單 subscriptions 已回讀；新增 `refunds/create` 必須在部署後同步並回讀，才算六個 topics 完整 |
...
3. `create-shopify-cart` 與目前 production `shopify-orders-webhook` 已部署；本 branch 新增退款折讓後須升版部署 webhook，並以 `npm run shopify:webhooks` 安全建立／回讀 `REFUNDS_CREATE`。`SAENGAK Order Sync` 只安裝於 Saengak、scope 為 `read_orders`；client secret 只保存於 Supabase。取得 `checkoutUrl` 後才導向 Shopify Checkout，付款與退款結果只接受已驗證 HMAC 的 Shopify webhook。
