...
> 更新日期：2026-08-23 (對齊 `00_DECISION_LOG.md` §3.4：Shopify Checkout 改為強制會員登入與可靠歸戶；自建 React Checkout／TapPay Direct Pay／`transaction_logs` 狀態機已廢棄；營運後台路由與 RLS 已接線)
...
| 結帳身分 | 強制會員登入與訂單歸戶 | 結帳前必須登入或註冊 Supabase 會員。`api/create-shopify-cart.ts` 以 `getUser()` 驗證 Bearer session，先寫入 `shopify_checkout_links`，成功後才建立帶有會員綁定 token 的 Shopify Cart；未登入、session 失效或綁定失敗一律停止結帳。 | 現行（2026-08-23） |
