...
- `create-shopify-cart` Edge Function 驗證 Origin、商品行數、variant ID 與數量，再呼叫 Shopify `cartCreate`。
...
- `orders`／`order_items`／`order_fulfillments` 維持會員唯讀；本機 24 項程式測試及 49 項 PostgreSQL 權限／同步測試已通過。
...
- 同日對 `gh2xgs-zf.myshopify.com` 的公開首頁與 Storefront API 做唯讀回讀：首頁導向 `/password`，GraphQL 回覆 `Online Store channel is locked`。因此程式已改接新商店，但在 Shopify 解鎖前無法取得商品或建立 checkoutUrl。
...
- Shopify 2026-07 Storefront API 的 tokenless probe 已到達該商店，但回覆 `Online Store channel is locked`；這表示 domain 與 API version 正確，仍需商店擁有人選擇付費方案並啟用 Online Store。
