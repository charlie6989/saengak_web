...
* **[最高權威決策總表 (00_DECISION_LOG.md)](file:///c:/Projects/saengak_web/docs/00_DECISION_LOG.md)**
  ...
  * **說明**: 規範定案架構（Phase 1 商品展示與部署聚焦、SiteGiant Shopify App 庫存雙向同步、ShipAny Shopify App 物流履行、自建 Checkout Option B 與 TapPay Direct Pay SDK 保留至 Phase 2）。
...
* **[結帳與交易安全性規格書 (CHECKOUT_PAYMENT_SPEC.md)](file:///c:/Projects/saengak_web/docs/CHECKOUT_PAYMENT_SPEC.md)**
  * **路徑**: `docs/CHECKOUT_PAYMENT_SPEC.md`
  * **說明**: TapPay SDK 整合、卡號零落地 PCI-DSS 規範、Shopify App 庫存聯動與 `transaction_logs` 補償機制 (Phase 2)。
...
* **[結帳頁面架構決策 (CHECKOUT_ARCHITECTURE_DECISION.md)](file:///c:/Projects/saengak_web/docs/decisions/CHECKOUT_ARCHITECTURE_DECISION.md)**
  * **路徑**: `docs/decisions/CHECKOUT_ARCHITECTURE_DECISION.md`
  * **決策**: 採「自建結帳頁面 (Option B)」，將商業邏輯與交易協同集中於 Vercel Serverless 中樞。
